...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     >,idists,ibroken,icount,input1,jsav,mt,ii1,ii2
     >,iclass,index,iii
     ...
     >,uno,eig1,eig2,qx,qy,disc,disc1,disc2,check,checkx
      ...
      REAL(8), dimension(:), allocatable :: point,momdist,kkx,kky,BBq
      REAL(8), dimension(:), allocatable :: dindip
      REAL(8), dimension(2,2) :: a,b,bb,enne1,amat
      REAL(8), dimension(:,:), allocatable :: x,y,ord,BBSq
...
C         kky(nmom)=qy
      ...
      allocate(momdist(nmom),kkx(nmom),kky(nmom)) 
      open(unit=95,file='rn_k.dat',form='formatted',status='unknown')
      open(unit=105,file='symmBZ.dat',form='formatted',status='unknown')
     
      do i=1, nmom
       read(95,*)kkx(i),kky(i),momdist(i)
      end do 

      do ii1=-2,2
       do ii2=-2,2
        do i=1,nmom    
         write(105,*)kkx(i)+ii1*bb(1,1)+ii2*bb(1,2), kky(i)
     > +ii1*bb(2,1)+ii2*bb(2,2),momdist(i) 
        enddo
       enddo  
      enddo

       allocate(BBq(nmom))
      open(unit=96,file='fourq.dat',form='formatted',status='unknown')
      open(unit=106,file='BZ_BBq.dat',form='formatted',status='unknown')

      do i=1, nmom
       read(96,*)kkx(i),kky(i),BBq(i)
      end do

      do ii1=-2,2
       do ii2=-2,2
        do i=1,nmom
         write(106,*)kkx(i)+ii1*bb(1,1)+ii2*bb(1,2), kky(i)
     > +ii1*bb(2,1)+ii2*bb(2,2),BBq(i)
        enddo
       enddo
      enddo

      allocate(BBSq(nmom,2))
      open(unit=97,file='bootsq.dat',form='formatted',status='unknown')
      open(unit=107,file='BZ_Sq.dat',form='formatted',status='unknown')

      do i=1, nmom
       read(97,*)kkx(i),kky(i),BBSq(i,1),qx,BBSq(i,2)
      end do

      do ii1=-2,2
       do ii2=-2,2
        do i=1,nmom
         write(107,*)kkx(i)+ii1*bb(1,1)+ii2*bb(1,2), kky(i)
     > +ii1*bb(2,1)+ii2*bb(2,2),BBSq(i,1)-BBSq(i,2)
        enddo
       enddo
      enddo
